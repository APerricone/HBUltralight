...
proc main()
    ...
    ///
    app_ := ultralight_App():Create()

    ///
    /// Create a resizable window by passing by OR'ing our window flags with
    /// kWindowFlags_Resizable.
    ///
    window_ = ultralight_Window():Create(app_:main_monitor, WINDOW_WIDTH, WINDOW_HEIGHT, ;
        .F., ulWindowFlags_Titled + ulWindowFlags_Resizable )
    ///
    /// Set the title of our window.
    ///
    window_:title := "Ultralight Sample 3 - Resize Me!"
    ...
    /// OnResize() below.
    ///
    left_pane_ := ultralight_overlay():Create(window_, 1, 1, 0, 0)
    right_pane_ := ultralight_overlay():Create(window_, 1, 1, 0, 0)

    ///
    /// Force a call to OnResize to perform initial layout and sizing of our
    /// left and right overlays.
    ///
    OnResize(window_, left_pane_, right_pane_, window_:width, window_:height)
    ...
    ///
    window_:bOnResize := {|width, height| OnResize(window_, left_pane_, right_pane_, width, height)}

    ///
    /// Register our MyApp instance as a ViewListener so we can handle the
    /// Views' OnChangeCursor event below.
    ///
    left_pane_:view:bOnChangeCursor := {|view,cursor| HB_SYMBOL_UNUSED(view), window_:cursor := cursor}
    right_pane_:view:bOnChangeCursor := left_pane_:view:bOnChangeCursor
    /// run the application
    app_:Run()
return

proc OnResize(window_, left_pane_, right_pane_, width, height)
    LOCAL left_pane_width_px, right_pane_width;

    left_pane_width_px := window_:DeviceToPixels(LEFT_PANE_WIDTH)
    left_pane_:Resize(left_pane_width_px, height)

    // Calculate the width of our right pane (window width - left width)
    right_pane_width := Max(width - left_pane_width_px, 1)

    right_pane_:Resize(right_pane_width, height)

    left_pane_:MoveTo(0, 0)
    right_pane_:MoveTo(left_pane_width_px, 0)
return

func htmlString_LeftPane()
    local v
